...
var _ = Describe("restore/data tests", func() {
	...
	Describe("CopyTableIn", func() {
		...
		It("will restore a table from a single data file", func() {
			execStr := regexp.QuoteMeta(fmt.Sprintf("COPY public.foo(i,j) FROM PROGRAM '(timeout 300 bash -c \"while [ ! -p \"<SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456\" ]; do sleep 1; done\" || (echo \"Pipe not found <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456\">&2; exit 1)) && cat <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456 | cat - && test ! -e \"%s_error\"' WITH CSV DELIMITER ',' ON SEGMENT;", fpInfo.GetSegmentPipePathForCopyCommand()))
			mock.ExpectExec(execStr).WillReturnResult(sqlmock.NewResult(10, 0))
	...
	Describe("CopyTableIn with resize restore from 4 to 3 segments", func() {
		...
		It("will restore a table from its own file with gzip compression", func() {
			utils.SetPipeThroughProgram(utils.PipeThroughProgram{Name: "gzip", OutputCommand: "gzip -c -1", InputCommand: "gzip -d -c", Extension: ".gz"})
			execStr := regexp.QuoteMeta(fmt.Sprintf("COPY public.foo(i,j) FROM PROGRAM '(timeout 300 bash -c \"while [ ! -p \"<SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_3456.gz\" ]; do sleep 1; done\" || (echo \"Pipe not found <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_3456.gz\">&2; exit 1)) && cat <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_3456.gz | cat - && test ! -e \"%s_error\"' WITH CSV DELIMITER ',' ON SEGMENT;", fpInfo.GetSegmentPipePathForCopyCommand()))
		...
		It("will restore a table from its own file with gzip compression using a plugin", func() {
			...
			pluginConfig := utils.PluginConfig{ExecutablePath: "/tmp/fake-plugin.sh", ConfigPath: "/tmp/plugin_config"}
			restore.SetPluginConfig(&pluginConfig)
			execStr := regexp.QuoteMeta(fmt.Sprintf("COPY public.foo(i,j) FROM PROGRAM '(timeout 300 bash -c \"while [ ! -p \"<SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456.gz\" ]; do sleep 1; done\" || (echo \"Pipe not found <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456.gz\">&2; exit 1)) && cat <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456.gz | cat - && test ! -e \"%s_error\"' WITH CSV DELIMITER ',' ON SEGMENT;", fpInfo.GetSegmentPipePathForCopyCommand()))
			mock.ExpectExec(execStr).WillReturnResult(sqlmock.NewResult(10, 10))
			mock.ExpectExec(execStr).WillReturnResult(sqlmock.NewResult(10, 10))
		...
		It("will restore a table from its own file with zstd compression using a plugin", func() {
			...
			pluginConfig := utils.PluginConfig{ExecutablePath: "/tmp/fake-plugin.sh", ConfigPath: "/tmp/plugin_config"}
			restore.SetPluginConfig(&pluginConfig)
			execStr := regexp.QuoteMeta(fmt.Sprintf("COPY public.foo(i,j) FROM PROGRAM '(timeout 300 bash -c \"while [ ! -p \"<SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456.zst\" ]; do sleep 1; done\" || (echo \"Pipe not found <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456.zst\">&2; exit 1)) && cat <SEG_DATA_DIR>/backups/20170101/20170101010101/gpbackup_<SEGID>_20170101010101_pipe_3456.zst | cat - && test ! -e \"%s_error\"' WITH CSV DELIMITER ',' ON SEGMENT;", fpInfo.GetSegmentPipePathForCopyCommand()))
			mock.ExpectExec(execStr).WillReturnResult(sqlmock.NewResult(10, 10))
			mock.ExpectExec(execStr).WillReturnResult(sqlmock.NewResult(10, 10))
