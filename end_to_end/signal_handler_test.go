...
var _ = Describe("Signal handler tests", func() {
	...
	Context("SIGINT", func() {
		...
		It("runs gprestore with copy-queue-size and sends a SIGINT to ensure cleanup functions successfully", func() {
			...
			_ = cmd.Run()
			outputRes := output.String()
			Expect(outputRes).To(ContainSubstring("Received an interrupt signal, aborting restore process"))
			Expect(outputRes).To(ContainSubstring("Cleanup complete"))
			Expect(outputRes).To(Not(ContainSubstring("CRITICAL")))
