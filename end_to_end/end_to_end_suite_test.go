...
LANGUAGE plpgsql NO SQL;`)
	...
	Describe("Backup partition these parent is in extension", func() {
		...
		It("ignore partitions whose parents are in extension in 6X if flag '--leaf-partition-data' is not provided.", func() {
			...
			testhelper.AssertQueryRuns(backupConn, `ALTER TABLE t_part ADD PARTITION test_part START (10) END (20);`)
		...
		It("normally handles the '--include-table' during backup", func() {
			...
			timestamp := gpbackup(gpbackupPath, backupHelperPath, "--backup-dir", backupDir, "--leaf-partition-data",
				"--include-table", tablePartA,
				"--include-table", tablePartC)
			metadataFileContents := string(getMetdataFileContents(backupDir, timestamp, "metadata.sql"))
			if backupConn.Version.AtLeast("7") {
				Expect(metadataFileContents).To(ContainSubstring("CREATE TABLE public.part_c"))
				Expect(metadataFileContents).To(ContainSubstring("ALTER TABLE ONLY public.t_part_1_prt_1 ATTACH PARTITION public.part_c FOR VALUES IN ('c');"))
				Expect(metadataFileContents).To(ContainSubstring("CREATE TABLE public.test_part"))
				Expect(metadataFileContents).To(ContainSubstring("ALTER TABLE ONLY public.t_part ATTACH PARTITION public.test_part FOR VALUES FROM (10) TO (20);"))
				Expect(metadataFileContents).To(ContainSubstring("CREATE TABLE public.part_a"))
				Expect(metadataFileContents).To(ContainSubstring("ALTER TABLE ONLY public.test_part ATTACH PARTITION public.part_a FOR VALUES IN ('a');"))
				Expect(metadataFileContents).ToNot(ContainSubstring("part_b"))
			} else {
			...
			defer testhelper.AssertQueryRuns(restoreConn, `DROP EXTENSION test_ext6 CASCADE;`)
		...
		It("preserve random-generated partition names", func() {
			if backupConn.Version.AtLeast("7") {
				Skip("not relevant for 7X")
			}
			testhelper.AssertQueryRuns(backupConn, "ALTER TABLE t_part ALTER PARTITION FOR (RANK(1)) ADD PARTITION VALUES ('c');")
			testhelper.AssertQueryRuns(backupConn, "INSERT INTO t_part SELECT a, 0, a, 'c' FROM generate_series(1, 100)a;")
			timestamp := gpbackup(gpbackupPath, backupHelperPath, "--backup-dir", backupDir, "--leaf-partition-data")
			gprestore(gprestorePath, restoreHelperPath, timestamp, "--backup-dir", backupDir, "--redirect-db", "restoredb")
		...
		It("7X backup data without the '--leaf-partition-data' flag", func() {
			if backupConn.Version.Before("7") {
